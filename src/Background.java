...
/**
 * It intializes the background with HUD for the life, score, and timer systems to appear on the game. 
...
/**
 * It's how the timer it's run and animation of the actual timer being runned.
 * Additionally, It adds whatever time is left to the score once it's over when the character finishes the level. 
 */
...
/**
 * It updates a new score whenever does an action to add to thier score in middle of the level like elimating an enemy as an example. 
 * As well, at the end wheneever the character reaches and all the elements like timer adds to it, it updates the entire score for the game. 
 * @param newScore is a variable that gets a new score itself whenever an action occurs to get a new score for the game. 
 */
...
/**
 * It updates the life whenever the character touches the enemies by the side in which it would decrease a life.
 * In which,  it decreases the life counter in the game
 * @param newLives is a variable that get a new live itself whenever the action occurs to affect a life for the character. 
 */
...
/**
 * It creates the camera's movement to follow the character
 * @param offsetX
 */
